...
class SynonymHandler
{
	...
	public static String[] removeSynonymLine (String[] synonymData,
	    String word) throws IllegalArgumentException
	{
        int indexOfLineToRemove = synonymLineIndex(synonymData, word);

        String[] newData = new String[synonymData.length - 1];

        for (int i = 0; i < indexOfLineToRemove; i++) {
            newData[i] = synonymData[i];
        }
        for (int i = indexOfLineToRemove; i < synonymData.length - 1; i++) {
            
            newData[i] = synonymData[i + 1];
        }
        return newData;
	}
	...
	{
        String synonyms = synonymLine.split("[|]")[1];

        String[] synonymList = synonyms.split("[,]");
        
        for (int i = 0; i < synonymList.length; i++) {
            synonymList[i] = synonymList[i].trim();
        }
	...
	{
        int index = synonymLineIndex(synonymData, word);
        synonymData[index] = String.join(", ", synonymData[index], synonym);
	}
	...
	{
        int index = synonymLineIndex(synonymData, word);
        String[] originalSyns = getSynonyms(synonymData[index]);
        String[] newSyns = new String[originalSyns.length - 1];
        int posOfSyn = 100;

        for (int i = 0; i < originalSyns.length; i++) {
            
            if (originalSyns[i].equals(synonym)) {
                posOfSyn = i;
            }
        }

        for (int i = 0; i < posOfSyn; i++) {
            newSyns[i] = originalSyns[i];
        }
        for (int i = posOfSyn; i < originalSyns.length - 1; i++) {
            newSyns[i] = originalSyns[i + 1];
        }
        


        synonymData[index] = word + " | " + String.join(", ", newSyns);
	}
    ...
    private static void sortIgnoreCase (String[] strings)
    {
        for (int i = 0; i < strings.length - 1; i++) {
            ...
            for (int j = i + 1; j < strings.length; j++) {
                if (strings[j].compareToIgnoreCase(strings[minPos]) < 0) {
                    minPos = j;
                }
            }

            String swapString = strings[i];
            strings[i] = strings[minPos];
            strings[minPos] = swapString;
        }
	}
    ...
    private static String sortSynonymLine (String synonymLine)
    {
        ...
        return synonymLine.split("[ ]")[0] + " | " + String.join(", ", synonyms);
	}
